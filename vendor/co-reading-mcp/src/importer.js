...
async function readImportedManifest(stdout, options) {
  ...
  const bookDir = options.bookId
    ? resolveInside(booksDir, options.bookId)
    : path.isAbsolute(importedPath)
      ? path.resolve(importedPath)
      : path.resolve(ROOT, importedPath);
...
async function runImport(filePath, options) {
  ...
  const manifest = await readImportedManifest(stdout, options);
